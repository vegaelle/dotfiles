      ...
      while [ $# -gt 0 ] ; do
        case $1 in
         FOC*)
           ...
           ;;
         URG*)
           wsp="${wsp}%{F${color_head} B${color_wsp_urg}}${sep_right}%{F${color_back} B${color_wsp_urg} T1} ${1#???} %{F${color_wsp_urg} B${color_head}}${sep_right}"
           ;;
         INA*)
           wsp="${wsp}%{F${color_disable}B${color_sec_b2} T1} ${1#???} "
             ;;
         ACT*)
